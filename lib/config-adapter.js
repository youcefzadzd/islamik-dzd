...
const clampCompanionMax = (v) =>
  Math.max(0, Math.min(10, Math.trunc(Number(v) || 0)));

/* ---------- companions section texts (fr / ar / en) ---------- */
const COMPANION_TEXTS = {
  fr: {
    title: "Accompagnants",
    addAdult: "+ Ajouter un accompagnant adulte",
    addChild: "+ Ajouter un enfant",
    nameLabel: "Nom complet de l’accompagnant",
    adult: "Adulte",
    child: "Enfant",
    remove: "Supprimer",
    // {n} = الحد الذي اختاره المالك في لوحة التحكم
    limitNote: "Vous pouvez ajouter jusqu’à {n} accompagnants.",
    noChildrenNote: "Nous sommes désolés, les enfants ne sont pas inclus dans cette invitation.",
    maxReached: "Vous avez atteint le nombre maximum d’accompagnants.",
    missingName: "Veuillez saisir le nom complet de chaque accompagnant.",
  },
  ar: {
    title: "المرافقون",
    addAdult: "+ إضافة مرافق بالغ",
    addChild: "+ إضافة طفل",
    nameLabel: "الاسم الكامل للمرافق",
    adult: "بالغ",
    child: "طفل",
    remove: "حذف",
    // {n} = الحد الذي اختاره المالك في لوحة التحكم
    limitNote: "يمكنك إضافة {n} مرافقين كحد أقصى.",
    noChildrenNote: "نعتذر، لا يمكن إضافة الأطفال في هذه الدعوة.",
    maxReached: "وصلت إلى الحد الأقصى للمرافقين.",
    missingName: "يرجى كتابة الاسم الكامل لكل مرافق.",
  },
  en: {
    title: "Companions",
    addAdult: "+ Add adult companion",
    addChild: "+ Add child",
    nameLabel: "Companion full name",
    adult: "Adult",
    child: "Child",
    remove: "Remove",
    // {n} = الحد الذي اختاره المالك في لوحة التحكم
    limitNote: "You can add up to {n} companions.",
    noChildrenNote: "Sorry, children are not included in this invitation.",
    maxReached: "You have reached the maximum number of companions.",
    missingName: "Please enter the full name of each companion.",
  },
};
...
/* الحرف الأول من أسماء العروسين كبير دائمًا في العرض:
   "youcef" → "Youcef"، "AMINE" → "Amine" — العربية تمرّ كما هي */
const formalName = (s) =>
  typeof s === "string"
    ? s
        .toLowerCase()
...
function buildData(lang, config) {
  ...
  /* ترتيب الاسمين يتبع جنس صاحب/صاحبة الدعوة: رجل → العريس أولًا
     (الافتراضي)، امرأة → العروس أولًا — في كل مواضع عرض الثنائي
     (الهيرو، التذييل، حروف الختم، عنوان التقويم...) */
  let couple =
    lang === "ar"
      ? {
          groomName: formalName(config.couple.groomNameAr),
          brideName: formalName(config.couple.brideNameAr),
          groomNameAr: formalName(config.couple.groomName),
          brideNameAr: formalName(config.couple.brideName),
        }
      : {
          ...config.couple,
          groomName: formalName(config.couple.groomName),
          brideName: formalName(config.couple.brideName),
          groomNameAr: formalName(config.couple.groomNameAr),
          brideNameAr: formalName(config.couple.brideNameAr),
        };
  if ((config.texts.invitation || {}).honoreeGender === "female") {
    couple = {
      ...couple,
      groomName: couple.brideName,
      brideName: couple.groomName,
      groomNameAr: couple.brideNameAr,
      brideNameAr: couple.groomNameAr,
    };
  }
  ...
  return {
    ...
    schedule: {
      ...
      items: config.program.map((step) => ({
        ...
        description:
          (lang === "ar" ? step.description_ar : step.description_fr) || undefined,
      })),
    },
    ...
    rsvp: {
      ...
      companions: {
        enabled: config.rsvp.allowCompanions === true,
        maxAdults: clampCompanionMax(config.rsvp.maxAdultCompanions ?? config.rsvp.maxCompanions),
        childrenAllowed: config.rsvp.childrenAllowed === true,
        maxChildren: clampCompanionMax(
          config.rsvp.maxChildren ??
            (config.rsvp.childrenAllowed === true ? config.rsvp.maxCompanions : 0)
        ),
        t: COMPANION_TEXTS[lang] || COMPANION_TEXTS.en,
      },
      /* تنويه «بدون أطفال» — يظهر بأناقة أعلى نموذج التأكيد عند تفعيله */
      noKidsNote:
        config.rsvp.noKidsNote === true
          ? lang === "ar"
            ? "نعتذر بلطف عن اصطحاب الأطفال — حفلنا للكبار فقط"
            : "Cérémonie réservée aux adultes — merci de venir sans enfants"
          : null,
    ...
    /* Wedding Invitation section (Heritage) — the owner's own words for
       the current language, plus the language-independent honoreeGender
       (male|female) that drives the gendered phrase; null when the owner
       filled nothing at all */
    invitation: (() => {
      const invTexts = config.texts.invitation || {};
      const group = invTexts[lang] || null;
      const honoreeGender =
        invTexts.honoreeGender === "male" || invTexts.honoreeGender === "female"
          ? invTexts.honoreeGender
          : null;
      if (!group && !honoreeGender) return null;
      return { ...(group || {}), honoreeGender };
    })(),
    ...
    assets: {
      ...
      heroVideo: config.media.heroVideo || "",
