...
/* الافتراضي الجديد: خانة المرافقين ظاهرة (3 بالغين + طفلان) لكل عرس
   لم يضبط أصحابه الإعداد صراحة؛ من عطّلها من اللوحة تبقى معطلة */
export const DEFAULT_RSVP_SETTINGS = {
  allow_companions: true,
  max_adult_companions: 3,
  children_allowed: true,
  max_children: 2,
  max_companions: 5, // legacy combined limit, kept in sync (adults + children)
};
...
export function normalizeRsvpSettings(input) {
  ...
  const children = childrenRaw === undefined ? unset : childrenRaw === true;
  ...
  const maxChildren = clamp10(
    input.max_children ??
      input.maxChildren ??
      (children ? legacyMax : 0) ??
      (unset ? DEFAULT_RSVP_SETTINGS.max_children : 0)
  );
  return {
