...
/**
 * Heritage-styled RSVP form, matching the reference screenshots: white
 * card, boxed rounded inputs with labels above, radio-style attendance
 * choices and a full-width dark-olive submit button.
 *
 * Same submission contract as the Islamic Royal RsvpSection
 * (rsvp_responses insert, companions, counts kept in sync).
 */
...
/* elegant "adults only" notice (Heritage template chrome) — shown when the
...
export default function HeritageRsvp({ data, sansClass = "" }) {
  ...
  /* form typography: the clear content font per language (Montserrat for
     FR, Amiri for AR — both already loaded); the script font never
     appears inside the form */
  const formFont = data.lang === "ar" ? "font-arabicText" : sansClass;
  const labelCls = `mb-2.5 block text-[15px] font-medium leading-[1.4] text-[#34372F] ${formFont}`;
  const boxCls = `w-full rounded-xl border border-ink/15 bg-white px-4 py-3 text-[16px] font-normal leading-relaxed text-[#2F332B] outline-none transition-colors placeholder:text-[15px] placeholder:font-normal placeholder:text-[rgba(47,51,43,0.52)] focus:border-gold ${formFont}`;

  return (
    <div className="mx-auto max-w-md rounded-[28px] border border-gold/15 bg-white/95 px-6 py-10 shadow-[0_18px_50px_rgb(64_47_32_/_0.1)] sm:px-9">
      <AnimatePresence mode="wait">
        {status === "success" ? (
          ...
          >
            <span
              aria-hidden
              className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-burgundy/15 text-3xl text-burgundy-dark"
            >
              ❦
            </span>
          ...
          >
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                {["yes", "no"].map((val) => (
                  <button
                    ...
                    className={`h-btn flex flex-1 items-center gap-2.5 rounded-xl border px-4 py-3 text-start text-[15px] font-medium leading-[1.4] ${formFont} ${
                      form.attending === val
                        ? "border-gold bg-[#FBF7EE] text-[#2F332B]"
                        : "border-ink/15 text-[#3B3D35] hover:border-gold/50"
                    }`}
                  >
                    <span
                      aria-hidden
            ...
            {form.attending === "yes" && companionsCfg.enabled && (
              <div>
                ...
                {!companionsCfg.childrenAllowed && (
                  <div className="mb-6 rounded-2xl border border-gold/50 bg-[#FBF7EE] px-5 py-7 text-center shadow-[0_8px_28px_rgb(64_47_32_/_0.06)]">
                    <span
                      aria-hidden
                      className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-gold-light/50 text-gold-dark"
                    >
                      {/* no-children glyph: a small child silhouette crossed
                          by one thin, gentle diagonal — refined, never a
                          harsh warning sign */}
                    ...
                    <p className={`mx-auto mt-3 max-w-xs text-[14.5px] leading-[1.7] text-[rgba(52,55,47,0.75)] ${formFont}`}>
                ...
                {atMax ? (
                  <p className={`text-[13px] leading-[1.5] text-[rgba(52,55,47,0.7)] ${formFont}`}>{ct.maxReached}</p>
                ) : (
                  <div className="flex flex-col gap-2 sm:flex-row">
                    {!atAdultMax && (
                      <button
                        ...
                        className={`h-btn flex-1 rounded-xl border border-dashed border-gold/40 px-4 py-2.5 text-[14px] font-medium leading-[1.4] text-[#796B5C] hover:bg-gold-light/20 ${formFont}`}
                      >
                        {ct.addAdult}
                      </button>
                    )}
            ...
            <div className="pt-1">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className={`flex w-full items-center justify-center gap-2.5 rounded-xl bg-burgundy px-8 py-4 text-[17px] font-semibold tracking-[0.02em] text-white shadow-[0_10px_28px_rgb(46_58_34_/_0.28)] transition-all duration-300 hover:bg-burgundy-dark hover:shadow-[0_12px_32px_rgb(46_58_34_/_0.35)] disabled:opacity-70 ${formFont}`}
              >
